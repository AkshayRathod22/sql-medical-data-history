...
-- Task 1.Show first name, last name, and gender of patients who's gender is 'M'
select first_name,last_name,gender
from patients
where gender="M";

--  Task 2.Show first name and last name of patients who does not have allergies.
...
-- Task 5.Update the patients table for the allergies column. 
...
-- Task 17.Show unique first names from the patients table which only occurs once in the list.
select first_name 
from patients
group by first_name
...
select patient_id,diagnosis,count(*) as patients_admitted_mul_times
from admissions
...
select day(admission_date) as day_months,count(*) as total_admission
from admissions
...
case when weight/((height/100)*(height/100))>30 then 1
else 0
end as isObese
from patients;
